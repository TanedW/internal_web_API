  ...
  export default async function handler(req, res) {
    ...
    try {
      ...
      // POST: จัดการ Multi-role (Add New / Reactivate / Append Roles)
if (req.method === 'POST') {
  ...
  const incomingRoles = roles.filter(r => validRolesList.includes(r.toLowerCase()));
  if (incomingRoles.length === 0) incomingRoles.push('editor');

  // 2. ตรวจสอบ User ใน Database
  const { rows: existing } = await db.query('SELECT * FROM admin_system WHERE email = $1 LIMIT 1', [email]);
  
  let targetUser;
  let actionStatus = 'ADMIN_CREATE_NEW';

  if (existing.length > 0) {
    targetUser = existing[0];
    if (targetUser.is_deleted) {
      // กรณีเคยถูกลบ ให้ดึงกลับมา (Reactivate)
      const { rows: updated } = await db.query(`UPDATE admin_system SET is_deleted = false WHERE email = $1 RETURNING *;`, [email]);
      targetUser = updated[0];
      actionStatus = 'ADMIN_REACTIVATE';
    } else {
      // กรณีมีตัวตนอยู่แล้ว ให้บันทึกว่าเป็นการเพิ่มสิทธิ์
      actionStatus = 'ADMIN_APPEND_ROLE';
    }
  } else {
  ...
  // --- Sync กับ Permit.io แบบ Append Only ---
  try {
    await permit.api.users.sync({
      key: String(targetUser.admin_id),
      email: targetUser.email,
    ...
    // ก) ดึงบทบาทปัจจุบันใน Permit ออกมา (ถ้ามี)
    const currentAssigned = await permit.api.users.getAssignedRoles({ 
        user: String(targetUser.admin_id), 
        tenant: "default" 
    });
    ...
    // ข) เพิ่มเฉพาะบทบาทใหม่ที่ยังไม่มี (ไม่มีการสั่ง Unassign ของเก่า)
    const newlyAdded = [];
    ...
    if (actorAdmin) {
        await saveAdminLog({
          ...
          details: { 
            ...
            added_roles: newlyAdded,
            all_roles: [...new Set([...currentRoleKeys, ...newlyAdded])]
          }
        });
    }
    ...
    return res.status(200).json({ 
      ...targetUser, 
      roles: [...new Set([...currentRoleKeys, ...newlyAdded])] 
    });

  } catch (e) {
    console.error("Permit Append Role Error:", e);
    return res.status(500).json({ message: 'Error syncing roles with Permit.io' });
  }
}
